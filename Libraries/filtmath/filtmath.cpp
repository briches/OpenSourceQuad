...
{
	int 	len,		// The number of elements in the x-array !!! X-Must be 1D
			na,			// The number of elements in the B array
	...
	/// Find row and col vector to create sparse matrix
	int rows[3 * nfilt - 5];
	int cols[3 * nfilt - 5];
	int data[3 * nfilt - 5];
	double sparse[3 * nfilt - 5][3 * nfilt - 5];

	// Fill the row array with the addresses
	...
	// Fill a 2D matrix with the data.
	for (int i = 0; i < ((3 * nfilt) - 5);i++)
	{	// sparse[col][row]
		sparse[col[i]][row[i]] = data[i];
	}
